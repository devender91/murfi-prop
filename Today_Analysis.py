...
authenticator = stauth.Authenticate(names, usernames, hashed_passwords, "Murfi Dashboard","abcde",cookie_expiry_days=30)
...
if authentication_status:
    ...
    with tabs[0]:
        ...
        date1 = pd.to_datetime(datetime.now() - timedelta(days=2))
        date2 = pd.to_datetime('today')
        
    
        with col1:
            st.markdown('<p style="font-size:32px; color:blue; font-weight:bold;">Today Analysis</p>', unsafe_allow_html=True)
        ...
        Colony = st.sidebar.multiselect("Pick the Colony", df2["Colony"].unique())
        if not Colony:
            df3 = df2.copy()
        else:
        ...
        Vendor = st.sidebar.multiselect("Pick the Vendor",df3["Vendor"].unique())
